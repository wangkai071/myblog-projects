    ...
    public partial class Form1 : Form
    {
                        ...
                        if (InvokeRequired)
                        {
                            Invoke(new Action(() =>
                            {
                                DialogResult result = MessageBox.Show(
                                    $"未找到运行中的WinCC画面({processName})，是否退出驱动程序？",
                                    "WinCC运行画面未打开",
        ...
        private void ProcessReceivedData(SerialPortContext context)
        {
            try
            {
                ...
                while (context.ReceiveBuffer.Count >= 5) // 最小帧长度
                {
                    ...
                    {
                        // 错误响应帧最小长度是5字节
                        int errorFrameLength = 5;

                        // 如果有额外数据，尝试确定完整帧长
                        if (context.ReceiveBuffer.Count >= errorFrameLength)
                        {
                            byte[] errorFrame = context.ReceiveBuffer.Take(errorFrameLength).ToArray();

                            // 验证CRC
                            byte[] crc;
                            CRC_16(errorFrame.Take(3).ToArray(), out crc);
                            if (crc[0] == errorFrame[3] && crc[1] == errorFrame[4])
                            {
                                ProcessCompleteFrame(errorFrame, context);
                                context.ReceiveBuffer.RemoveRange(0, errorFrameLength);
                                continue;
                            }
                            else
                            {
                                // CRC不匹配，可能是损坏的帧，尝试查找下一个帧头
                                int nextStart = FindNextFrameStart(context.ReceiveBuffer);
                                if (nextStart > 0)
                                {
                                    // 记录CRC校验失败
                                    string crcError = $"{DateTime.Now:HH:mm:ss.fff} [{context.Prefix}-{context.Port.PortName}] CRC校验失败，尝试查找下一帧头\r\n";
                                    Debug.WriteLine(crcError);

                                    if (!isPaused && !globalCts.IsCancellationRequested)
                                    {
                                        this.BeginInvoke(new Action(() =>
                                        {
                                            richTextBox1.AppendText(crcError);
                                           // richTextBox1.ScrollToCaret();
                                        }));
                                    }
                                ...
                                {
                                    // 没有找到有效帧头，清空缓冲区
                                    string clearBuffer = $"{DateTime.Now:HH:mm:ss.fff} [{context.Prefix}-{context.Port.PortName}] 未找到有效帧头，清空缓冲区\r\n";
                                    ...
                                    if (!isPaused && !globalCts.IsCancellationRequested)
                                    {
                                        this.BeginInvoke(new Action(() =>
                                        {
                                            richTextBox1.AppendText(clearBuffer);
                                           // richTextBox1.ScrollToCaret();
                                        }));
                                    }
        ...
        private int FindNextFrameStart(List<byte> buffer)
        {
            for (int i = 0; i < buffer.Count - 1; i++)
            {
                ...
                if (buffer[i] >= 1 && buffer[i] <= 247)
                {
                    ...
                    // 检查是否是有效的功能码
                    bool isValidFunction =  functionCode == 0x03 ||
                                            functionCode == 0x06 ||
                                            functionCode == 0x10 ||
                                           (functionCode  & 0x80)!= 0; // 错误响应

                    if (isValidFunction)
                    {
                        // 进一步验证CRC（如果缓冲区足够长）
                        if (i + 5 < buffer.Count) // 至少需要5字节才能验证CRC
                        {
                            byte[] possibleFrame = buffer.Skip(i).Take(5).ToArray();
                            byte[] calculatedCrc;
                            CRC_16(possibleFrame.Take(3).ToArray(), out calculatedCrc);

                            if (possibleFrame[3] == calculatedCrc[0] &&
                                possibleFrame[4] == calculatedCrc[1])
                            {
                                return i;
                            }
                        }
            ...
            try
            {
                ...
                try
                {
                    if (!stopWritingToWinCC && !globalCts.IsCancellationRequested)
                    {
                        ...
                        // 同时更新到调试显示
                        Debug.WriteLine($"{context.Prefix}区（{context.Port.PortName}）收到: {rawDataHex}");
                    }
                }
                catch (Exception ex)
                {
                ...
                if (!isModbusResponse)
                {
                    ...
                    try
                    {
                        ...
                        {
                            string messageTag = $"meiqi{context.Prefix}1";
                            string errorMessage = $"{timestamp}=>错误响应: {rawDataHex}";
                            hmi485.Tags[messageTag].Write(errorMessage);
                        }
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"写入WinCC错误响应失败: {ex.Message}");
                    }
                }
        ...
        private void CheckFrameTimeout(SerialPortContext context)
        {
            lock (context.SerialLock)
            {
                ...
                {
                    // 记录详细的超时信息
                    string timeoutText = $"{DateTime.Now:HH:mm:ss.fff} [{context.Prefix}-{context.Port.PortName}] 帧超时，丢弃{context.ReceiveBuffer.Count}字节: {BitConverter.ToString(context.ReceiveBuffer.ToArray()).Replace("-", " ")}\r\n";

                    Debug.WriteLine(timeoutText);
        ...
        private byte[] BuildModbusRequest(int address, int startRegister, int registerCount, SerialPortContext context)
        {
            ...
            try
            {
                if (!stopWritingToWinCC && !globalCts.IsCancellationRequested)
                {
                    ...
                    // 同时更新到调试显示
                    Debug.WriteLine($"{context.Prefix}区{context.Port.PortName}请求: {hexString}");
                }
            }
            catch (Exception ex)
            {
        ...
        private void ProcessRegisterValue(int address, int registerNumber, int value, SerialPortContext context)
        {
            ...
            try
            {
                if (!stopWritingToWinCC && !globalCts.IsCancellationRequested)
                {
                    ...
                    string BoolTag   = $"{context.Prefix}{registerNumber}_Bool";
                    ...
                    else // 正常值
                    {
                        ...
                        // 正常值不添加到报警消息中
                        if (actualValue>25.0|| actualValue<0.0)
                        {
                            hmi485.Tags[BoolTag].Write(1);
                        }
                        else 
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            foreach (var context in serialPortContexts)
            {
                for (int i = 1; i <= context.RegisterCount; i++)
                {
                    string boolTag = $"{context.Prefix}{i}_Bool";
                    hmi485.Tags[boolTag].Write(0);
                }
            }
                globalCts.Cancel();
        ...
        private void richTextBox1_TextChanged(object sender, EventArgs e)//滚屏
        {
            ...
            if (richTextBox1.Lines.Length > maxLine)
            {
                ...
                // 移除最早的行直到行数在限制范围内
                int linesToRemove = richTextBox1.Lines.Length - maxLine;
                int removeUpToIndex = 0;

                for (int i = 0; i < linesToRemove; i++)
                {
                    removeUpToIndex += richTextBox1.Lines[i].Length + 1; // +1 for newline
                }

                // 保留用户选择位置
                if (selectionStart > removeUpToIndex)
                {
                    selectionStart -= removeUpToIndex;
                }
                else
                {
                    selectionStart = 0;
                ...
                // 移除文本
                richTextBox1.Text = richTextBox1.Text.Substring(removeUpToIndex);

                // 恢复选择位置
                richTextBox1.SelectionStart = selectionStart;

                // 恢复滚动位置
                if (firstVisibleCharIndex > removeUpToIndex)
                {
                  //  richTextBox1.ScrollToCaret();
                }
            }
        ...
        private void button1_Click(object sender, EventArgs e) {  isPaused = !isPaused;  button1.Text = isPaused ? "打开滚屏" : "停止滚屏";   }
        private void textBox1_TextChanged(object sender, EventArgs e) { }

        private void button2_Click(object sender, EventArgs e)  {  this.WindowState = FormWindowState.Minimized; this.ShowInTaskbar = false; this.Visible = false;  }
